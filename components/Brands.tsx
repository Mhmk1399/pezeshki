...
const BrandsSlider: React.FC = () => {
  ...
  const handlePrev = () => {
    setCurrentIndex((prevIndex) => (prevIndex - 1 + Brandss.length) % Brandss.length);
  };

  const handleNext = () => {
    setCurrentIndex((prevIndex) => (prevIndex + 1) % Brandss.length);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto text-center mt-20 mb-10">
      <div className="flex items-center justify-between mb-4" dir='rtl'>
        <h2 className="text-3xl text-pink-600 mb-10">برند ها</h2>
        <div>
      ...
      <div className="overflow-hidden relative">
        ...
        >
          {Brandss.concat(Brandss).map((Brands, index) => (
            <div key={index} className="flex-none w-1/4 p-2">
              <div className="bg-white rounded-2xl shadow-md p-5">
                <div className="text-5xl mb-4">{Brands.icon}</div>
                <h3>{Brands.title}</h3>
              </div>
            </div>
          ))}
